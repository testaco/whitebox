 ...
 * Chris Testa, Algoram Inc:
 * - Moved to using PDMA driver
 ...
 */
#define SPI_PDMA_WRITE_ADJ      (0xF << 14)
...
static void spi_m2s_dma_cb(void *ptr, int buf);
...
static int spi_m2s_hw_init(struct m2s_spi_dsc *s)
{
	 ...
	 * Configure DMA
	 */
    pdma_request(s->drx, spi_m2s_dma_cb, spi_m2s_dsc_tbl[s->drx], 0xf,
            s->drx_sel |
            PDMA_CONTROL_SRC_ADDR_INC_0 |
            PDMA_CONTROL_XFER_SIZE_1B |
            PDMA_CONTROL_DIR_PERIPH_TO_MEM);

    pdma_request(s->dtx, 0, 0, 0xf,
            s->dtx_sel |
...
static inline int spi_m2s_hw_cs_set(struct m2s_spi_dsc *s, int cs)
{
    ...
    printk(KERN_INFO "SLAVE SELECT %d to %d\n", cs, v);
...
static void spi_m2s_xfer_start(struct m2s_spi_dsc *s)
{
	 ...
	 */
    if (s->xf->tx_buf) {
        printk(KERN_INFO "starting transmit\n");
        pdma_start(s->dtx, (u32)&M2S_SPI(s)->tx_data, (u32)s->xf->tx_buf, len);
    }
    if (s->xf->rx_buf) {
        printk(KERN_INFO "starting receive\n");
        pdma_start(s->drx, (u32)&M2S_SPI(s)->rx_data, (u32)s->xf->rx_buf, len);
    }
 done:
	return;
}
...
static void spi_m2s_dma_cb(void *ptr, int buf)
{
    ...
    printk(KERN_INFO "made it here3\n");
    ...
    printk(KERN_INFO "made it here4\n");
    ;
}
 ...
 * @returns	0->success, <0->error code
 */
static int spi_m2s_remove(struct platform_device *pdev)
{
